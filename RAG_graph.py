...
from inspection_report import _InspectionReportMixin
from utils import (
    average_embeds,
...
ANCHOR_PROP_DEFAULTS = {
    ...
    "prop_max_occurrences": 2000,   # cap on occurrences entering propagation; above this, FFT+random downsample then label the rest by nearest sampled neighbor (0/None disables)
}
...
class TokenNode:
    ...
    # Schema 9: reservoir-sampling bookkeeping. occurrence_count is the exact
    # number of occurrences seen during indexing (independent of embeds_buffer
    # truncation); embeds_buffer_indices holds the global occurrence index of
    # each retained sample in embeds_buffer (same order/length), used to map
    # sampled chunk_edge_weights back onto the full span_occurrences list.
    occurrence_count: int = 0
    embeds_buffer_indices: list = field(default_factory=list)
...
class LiteSemRAG(_InspectionReportMixin):
    ...
    def __init__(self,
        ...
        # Reservoir-sampling cap on per-token embeds_buffer during indexing.
        # None / <=0 keeps every occurrence's embedding (original behavior).
        # The full lightweight metadata (span_occurrences) is always kept, so
        # chunk links / df / IDF are unaffected; only the heavy 1024-d embeddings
        # are subsampled, which makes the center / s_mean sampled estimates.
        self.embeds_buffer_cap = (
            int(embeds_buffer_cap) if embeds_buffer_cap and int(embeds_buffer_cap) > 0 else None
        )
    ...
    # Accumulate one occurrence during indexing: the lightweight span occurrence
    # is always kept (full chunk/df/IDF metadata), while the heavy embedding is
    # reservoir-sampled into embeds_buffer when embeds_buffer_cap is set, so a
    # very high-frequency term cannot blow up memory. embeds_buffer_indices keeps
    # each retained sample's global occurrence index for later edge-weight mapping.
    def _accumulate_token_occurrence(self, token_node, text_embedding):
        self._append_token_occurrence(token_node, text_embedding)
        global_index = token_node.occurrence_count
        token_node.occurrence_count += 1
        cap = self.embeds_buffer_cap
        buffer = token_node.embeds_buffer
        if cap is None or len(buffer) < cap:
            buffer.append(text_embedding)
            token_node.embeds_buffer_indices.append(global_index)
            return
        # Reservoir sampling: replace a random slot with probability cap/(i+1).
        slot = random.randint(0, global_index)
        if slot < cap:
            buffer[slot] = text_embedding
            token_node.embeds_buffer_indices[slot] = global_index
    ...
    # Expand sampled chunk-edge weights to one entry per full occurrence.
    # sampled_sim[j] is the exact similarity of the sample whose global occurrence
    # index is sampled_indices[j]; occurrences that were not retained by reservoir
    # sampling get the sampled median as a neutral fallback so they still rank in
    # the middle when chunks are ordered by edge weight at query time. When no
    # sampling happened (len == n_full) every position is filled exactly.
    def _expand_edge_weights_to_full(self, sampled_sim, sampled_indices, n_full):
        if n_full <= 0:
            return []
        if not sampled_sim:
            return [0.0] * n_full
        fill = float(np.median(sampled_sim)) if len(sampled_indices) < n_full else 0.0
        full = [fill] * n_full
        for j, global_index in enumerate(sampled_indices):
            if 0 <= global_index < n_full and j < len(sampled_sim):
                full[global_index] = float(sampled_sim[j])
        return full

    # Create one semantic node for a token node. Chunk links / span occurrences
    # come from the complete span_occurrences list (so df / IDF / recall are
    # unaffected by reservoir sampling); the center embedding and retained
    # samples come from the possibly-subsampled embeds_buffer.
    def create_basic_sem_node(self, token_node):
        new_sem_node = self.create_sem_node(token_node)
        sampled = token_node.embeds_buffer
        full_occurrences = token_node.span_occurrences
        new_sem_node.chunk_node_embed = [te.embed for te in sampled]
        new_sem_node.embed = self._store_sem_embed(average_embeds(new_sem_node.chunk_node_embed))
        new_sem_node.chunk_node_list = [so.chunk_node for so in full_occurrences]
        new_sem_node.span_occurrences = list(full_occurrences)
        self._initialize_sem_retained_text_embeddings(new_sem_node, sampled)
        sampled_sim = sem_embed_sim(new_sem_node).cpu().tolist()
        new_sem_node.chunk_edge_weight = self._expand_edge_weights_to_full(
            sampled_sim, token_node.embeds_buffer_indices, len(full_occurrences)
        )
    ...
    def _build_sem_node_from_cluster(self, token_node, text_embeddings):
        ...
        # Occurrences dropped by reservoir sampling never reached the split
        # decision; attach them (chunk link + span) to the dominant sense so
        # their chunks stay retrievable. No-op when nothing was truncated.
        self._attach_unsampled_occurrences(token_node, final_sem_nodes)
        token_node.sem_node_list.extend(final_sem_nodes)

    # Attach occurrences that reservoir sampling dropped before the multi-sense
    # split to the largest resulting sense node, keeping chunk_node_list /
    # span_occurrences / chunk_edge_weight aligned. No-op when embeds_buffer held
    # every occurrence (no truncation).
    def _attach_unsampled_occurrences(self, token_node, sem_nodes):
        full_occurrences = token_node.span_occurrences
        sampled_set = set(token_node.embeds_buffer_indices)
        if not sem_nodes or len(sampled_set) >= len(full_occurrences):
            return
        target = max(sem_nodes, key=lambda node: len(node.chunk_node_list))
        fill = float(np.median(target.chunk_edge_weight)) if target.chunk_edge_weight else 0.0
        for global_index in range(len(full_occurrences)):
            if global_index in sampled_set:
                continue
            span_occurrence = full_occurrences[global_index]
            target.chunk_node_list.append(span_occurrence.chunk_node)
            target.span_occurrences.append(span_occurrence)
            target.chunk_edge_weight.append(fill)
    ...
    def build_sem_node(self, token_node):
        ...
        token_node.embeds_buffer_indices.clear()
    ...
    def _ensure_token_node_backward_compatible_attrs(self, token_node):
        ...
        if not hasattr(token_node, "llm_anchor_sample_assignments"):
            token_node.llm_anchor_sample_assignments = []
        # Schema 9: reservoir-sampling bookkeeping. Older pickles are already
        # finalized (embeds_buffer cleared), so default to a zero count / empty
        # index list; both are only consulted during finalize.
        if not hasattr(token_node, "occurrence_count"):
            token_node.occurrence_count = len(getattr(token_node, "embeds_buffer", []) or [])
        if not hasattr(token_node, "embeds_buffer_indices"):
            token_node.embeds_buffer_indices = list(
                range(len(getattr(token_node, "embeds_buffer", []) or []))
            )
    ...
    def finalize_token_nodes(self):
        ...
        for token_node in self.token_nodes:
            if token_node.has_semantic:
                continue
            # Exact occurrence count (not len(embeds_buffer), which may be capped
            # by reservoir sampling) so the multi-sense gate uses the real count.
            occurrence_count = getattr(token_node, "occurrence_count", 0) or len(token_node.embeds_buffer)
    ...
    # Pick up to `cap` representative positions for propagation: FFT covers the
    # boundary / rare points, the rest are random. Returns sorted unique indices.
    def _subsample_positions_for_propagation(self, embeddings, cap):
        n = len(embeddings)
        if cap >= n:
            return list(range(n))
        fft_ratio = float(self._anchor_param("anchor_fft_ratio"))
        random_state = int(self._anchor_param("anchor_random_state"))
        n_fft = min(cap, int(round(fft_ratio * cap)))
        n_rand = cap - n_fft
        fft_positions = []
        if n_fft > 0:
            fft_positions = [
        ...
        fft_set = set(fft_positions)
        remaining = [p for p in range(n) if p not in fft_set]
        rand_positions = []
        if n_rand > 0 and remaining:
            rng = np.random.default_rng(random_state)
            take = min(n_rand, len(remaining))
    ...
    # Label every unsampled global position by the label of its nearest sampled
    # neighbor (cosine). `sub_labels` is keyed by subset-local position; `labels`
    # (keyed by global position) is updated in place.
    def _fill_unsampled_labels_by_nearest(self, full_embeddings, sub_idx, sub_labels, labels):
        sub_set = set(sub_idx)
        rest = [g for g in range(len(full_embeddings)) if g not in sub_set]
        if not rest:
            return
        sub_norm = _l2_normalize_rows(full_embeddings[sub_idx].astype(np.float32))
        rest_norm = _l2_normalize_rows(full_embeddings[rest].astype(np.float32))
        nearest_local = np.argmax(rest_norm @ sub_norm.T, axis=1)
        for k, g in enumerate(rest):
            lab = sub_labels.get(int(nearest_local[k]))
            if lab is not None:
                labels[g] = lab
    ...
    def _assign_sem_description_anchor_propagation(self, sem_node, cluster_text_embeddings):
        ...
        full_embeddings = np.stack([self._te_embed_to_numpy(te) for te in records]).astype(np.float32)

        # Cap the occurrences entering propagation. Above prop_max_occurrences the
        # dense (mutual-)kNN matrix is O(n^2) and the per-occurrence cross-encoder
        # is O(n) CE calls, so a single very high-frequency term can blow up memory
        # and cost. Downsample with FFT+random, run the full anchor/CE/propagation
        # pipeline on the subset, then label every unsampled occurrence by its
        # nearest sampled neighbor. n <= cap keeps the original full-set behavior.
        prop_cap = int(self._anchor_param("prop_max_occurrences"))
        if prop_cap > 0 and n > prop_cap:
            sub_idx = self._subsample_positions_for_propagation(full_embeddings, prop_cap)
        else:
            sub_idx = list(range(n))
        sub_records = [records[i] for i in sub_idx]
        embeddings = full_embeddings[sub_idx]
        m = len(sub_records)

        anchor_positions = self._sample_anchor_positions(embeddings)
        candidate_bank, anchor_labels = self._label_anchor_positions(
            sem_node, sub_records, anchor_positions
        )
        ...
        if need_ce:
            ce_top1, ce_scores, ce_margin = self._full_cross_encoder_over_records(
                token_text, sub_records, candidate_bank
            )
        else:
            ce_top1, ce_scores, ce_margin = {}, {}, {}

        prop = self._run_anchor_propagation_core(
            sub_records, embeddings, anchor_labels, ce_top1, ce_scores, ce_margin, spec
        )
        sub_labels = prop["labels"]
        provenance = prop["provenance"]
        uncertain_positions = prop["uncertain_positions"]

        # Cleanup for uncertain items (subset-local positions).
        if spec["uncertain_mode"] == "ce_fallback":
            for pos in uncertain_positions:
                fallback = ce_top1.get(pos)
                if fallback is not None:
                    sub_labels[pos] = fallback
                    provenance[pos] = "uncertain_ce_fallback"
        elif spec["uncertain_mode"] == "re_llm":
            relabeled = self._relabel_uncertain_with_llm(
                token_text,
                [(pos, sub_records[pos]) for pos in uncertain_positions],
                candidate_bank,
            )
            for pos, description in relabeled.items():
                sub_labels[pos] = description
                provenance[pos] = "uncertain_re_llm"

        # Sampled positions still unlabeled after LLM/CE fallback failure -> take
        # the subset majority so every sampled occurrence carries a label before
        # it is propagated to the unsampled ones.
        if len(sub_labels) < m:
            if not sub_labels:
                sem_node.chunk_node_embed.clear()
                return [sem_node]
            majority_description = Counter(sub_labels.values()).most_common(1)[0][0]
            for pos in range(m):
                if pos not in sub_labels:
                    sub_labels[pos] = majority_description
                    provenance[pos] = "fallback_majority"

        # Map subset-local labels back to global positions, then label each
        # unsampled occurrence by its nearest sampled neighbor.
        labels = {sub_idx[local_pos]: lab for local_pos, lab in sub_labels.items()}
        if m < n:
            self._fill_unsampled_labels_by_nearest(
                full_embeddings, sub_idx, sub_labels, labels
            )
        if len(labels) < n:
            majority_description = Counter(labels.values()).most_common(1)[0][0]
            for pos in range(n):
                labels.setdefault(pos, majority_description)
    ...
    def process_embeds(self, new_chunk_node, phrase_embs, token_embs):
        for embed_record in phrase_embs + token_embs:
            ...
            self._accumulate_token_occurrence(token_node, text_embedding)
