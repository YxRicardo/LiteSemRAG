"""Evaluate Milestone 2 / Milestone 3 multi-hop on HotpotQA (distractor 500).

Loads an existing fastidx cache (no rebuild) and compares query-time methods:
  baseline : chunk_cooccur_query()    - single-hop baseline
  bridge   : multihop_bridge_query()  - Plan 1, chunk-local bridge extraction
  path     : multihop_path_query()    - Plan 2, global co-occurrence path bridge
  routed   : multihop_query()         - Plan 3, plan_query() strategy router
...
OUT = Path("reports/multihop_m2m3_eval_results.json")
...
def bridge_titles(db, question):
    _, chunk_ids, _ = db.multihop_bridge_query(
        question, top_k_chain=TOP_K, first_hop_k=20, bridge_top_k=8, second_hop_k=20,
    )
    return titles_from_chunk_ids(db, chunk_ids)


def path_titles(db, question):
...
def main():
    ...
    results = {"index_params": db._sem_cooccur_index_params}
    for name, fn in [
        ("baseline", baseline_titles),
        ("bridge", bridge_titles),
        ("path", path_titles),
        ("routed", routed_titles),
    ]:
